...
public class ParkingLotDataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        if (parkingLotRepository.count() == 0) {
            ...
            lot1.setApproved(true);
            lot1.setApprovedAt(LocalDateTime.now().minusDays(10));

            // Prices
            ParkingLotPrice price1a = createPrice(VehicleType.Car, new BigDecimal("2.50"), lot1);
            ParkingLotPrice price1b = createPrice(VehicleType.Motorcycle, new BigDecimal("1.00"), lot1);
            ...
            lot2.setApproved(true);
            lot2.setApprovedAt(LocalDateTime.now().minusDays(5));

            // Prices
            ParkingLotPrice price2a = createPrice(VehicleType.Car, new BigDecimal("3.00"), lot2);
            ParkingLotPrice price2b = createPrice(VehicleType.Motorcycle, new BigDecimal("1.50"), lot2);
            ParkingLotPrice price2c = createPrice(VehicleType.Truck, new BigDecimal("6.00"), lot2);
            ...
            lot3.setApproved(false);
            lot3.setApprovedAt(null);

            // Prices
            ParkingLotPrice price3a = createPrice(VehicleType.Car, new BigDecimal("2.00"), lot3);
            ParkingLotPrice price3b = createPrice(VehicleType.Motorcycle, new BigDecimal("0.80"), lot3);
            ...
            log.info("Mock parking lots, prices, spots, and add-ons successfully seeded.");
        }
    }

    private ParkingLotPrice createPrice(VehicleType vehicleType, BigDecimal price, ParkingLot parkingLot) {
    ...
    private ParkingSpot createSpot(VehicleType vehicleType, ParkingLot parkingLot) {
